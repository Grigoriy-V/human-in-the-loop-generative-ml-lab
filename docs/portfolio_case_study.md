...
The AFHQ comparison used 5,153 training cats and a separate 500-image held-out split. Each training source produced four deterministic `RandomResizedCrop(scale=0.85–1.0, ratio=1.0)` variants with alternate horizontal flips, while evaluation used one center-square crop without training augmentation; manifests and hashes tied latents to sources, and training never opened the test cache.
...
Portfolio packaging is complete. The Cats-to-all-AFHQ transfer remains planned, has not started, and stays human-gated; it would use the frozen early-stop checkpoint as a parent artifact. For evidence navigation and practical constraints, see the [README](../README.md), [technical retrospective](technical_retrospective.md), and [reproducibility guide](reproducibility.md).
